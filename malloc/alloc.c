...
//whether enough after can be improved
  meta_data* h = p->prev;
  meta_data* coales = NULL;
  size_t capacity = p->size;
  while(h!=NULL)
  {
    if(h->free == 0)
      break;
    capacity += (h->size+sizeof(meta_data));
    if(capacity >= size)
      {
      coales = h;
      break;
